...
K = 16;     % Sub-carriers                                              %此檔為ZF調變的IDFHE
M = 32;      % Sub-Symbols                                            
N = K*M ;  % N個samples                                                
...
%BER_Er6 =  zeros(1,length(SNR_range));
...
for abc = 1:N_ch % 通道跑的次數
    ...
    for  SNR_bit_dB = SNR_range
        ...
        Er = pinv(A)*pinv(H)*r;%ZF解調乘上r
        
        Sig_Er = qamdemod(Er,4);  % dataSymbol==Rx_Sig
       ...
       %%%%%%%%%E_CLS%%%%%%%%%%%%%%%%%
        ...
        %%%%%%%%%E_CLS_2%%%%%%%%%%%%%%%%%
         ...
         %%%%%%%%%E_CLS_3%%%%%%%%%%%%%%%%%
         Er4 = Eclsr-BI2*data_est_3;
         Sig_Er4 = qamdemod(Er4,4);
         ...
         Sig_Er5 = qamdemod(Er5,4);
         %data_est_5 = (qammod(Sig_Er5,M_mod))/sqrt(P).*(sqrt(SNR_symbol));%下一次疊代要扣除的ISI,先算好
         ...
         %%%%%%%%%E_CLS_5%%%%%%%%%%%%%%%%%
         %Er6 = Eclsr-BI2*data_est_5;
         %Sig_Er6 = qamdemod(Er6,4);
         %Sig_Er6 = reshape(Sig_Er6,N*N_GFDM,1);
         %dataOut_Er6 = de2bi(Sig_Er6,mm); 
         %nErrors_Er6 = biterr(dataIn,dataOut_Er6); 
         %BER_Er6(SNR_bit_dB/2-SNR_range(1)/2+1) = nErrors_Er6/(N_bits);
       ...
       %last_BER_Er6 =  BER_Er6+last_BER_Er6;
end      
...
%BER_Er6 = last_BER_Er6/N_ch ;

semilogy(SNR_range,BER_Er,'-o') %non-iterative ZF
xlabel('E_b/N_0(dB)','FontSize',15)
ylabel('BER','FontSize',15)
set(gca,'FontSize',12)
hold on
grid on
semilogy(SNR_range,BER_Er2,'-s') %ZF-based IDFHE (I=2 iteration)
semilogy(SNR_range,BER_Er3,'-^') %ZF-based IDFHE (I=3 iteration
semilogy(SNR_range,BER_Er4,'-x') %ZF-based IDFHE (I=4 iteration)
semilogy(SNR_range,BER_Er5,'--') %ZF-based IDFHE (I=5 iteration)
%semilogy(SNR_range,BER_Er6,'-*') 
legend('non-iterative ZF','ZF-based IDFHE (I=2 iteration)','ZF-based IDFHE (I=3 iteration)','ZF-based IDFHE (I=4 iteration)','ZF-based IDFHE (I=5 iteration)')
